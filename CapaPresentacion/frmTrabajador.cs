...
namespace CapaPresentacion
{
    public partial class frmTrabajador : Form
    {
        ...
        private void Limpiar()
        {
            ...
            this.txtIdtranajador.Text = string.Empty;
        }
        ...
        private void Habilitar(bool valor)
        {
            ...
            this.txtIdtranajador.ReadOnly = !valor;
        ...
        {
            this.txtIdtranajador.Text = Convert.ToString(this.dataListado.CurrentRow.Cells["idtrabajador"].Value);
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.IsNuevo)
                    {
                        rpta = NTrabajador.Insertar(this.txtNombre.Text.Trim().ToUpper(), this.txtApellidos.Text.Trim().ToUpper(), this.cbSexo.Text, dtFechaNac.Value,
                            txtNum_Documento.Text, txtDireccion.Text, txtTelefono.Text, txtEmail.Text, this.cbAcceso.Text, this.txtUsuario.Text, this.txtPassword.Text);
                    }
                    else
                    {
                        rpta = NTrabajador.Editar(Convert.ToInt32(this.txtIdtranajador.Text),
                            this.txtNombre.Text.Trim().ToUpper(), this.txtApellidos.Text.Trim().ToUpper(), this.cbSexo.Text, dtFechaNac.Value,
                            txtNum_Documento.Text, txtDireccion.Text, txtTelefono.Text, txtEmail.Text, cbAcceso.Text, this.txtUsuario.Text, this.txtPassword.Text);
                    }
                    if (rpta.Equals("OK"))
                    {
                        if (this.IsNuevo)
                        {
                            this.MensajeOk("Se Insertó de forma correcto el registro");
                        }
                        else
                        {
                            this.MensajeOk("Se Actualizó de forma correcto el registro");
                        }
                    }
                    else
                    {
        ...
        {
            if (!this.txtIdtranajador.Text.Equals(""))
            {
                this.IsEditar = true;
                this.Botones();
                this.Habilitar(true);
